...
fn add(machine: &mut Machine<f64, u32>, _args: &[usize]) {
...
fn sub(machine: &mut Machine<f64, u32>, _args: &[usize]) {
    let rhs = machine.operand_pop();
    let lhs = machine.operand_pop();
    machine.operand_push(lhs - rhs);
}
...
/// second and pushes the result back onto the stack.
fn div(machine: &mut Machine<f64, u32>, _args: &[usize]) {
    let rhs = machine.operand_pop();
    let lhs = machine.operand_pop();
    machine.operand_push(lhs / rhs);
}
...
/// pushes the result back onto the stack.
fn mult(machine: &mut Machine<f64, u32>, _args: &[usize]) {
    let rhs = machine.operand_pop();
    let lhs = machine.operand_pop();
    machine.operand_push(lhs * rhs);
}
...
fn instruction_table() -> InstructionTable<f64, u32> {
...
fn addition_example() {
    ...
    let mut builder: Builder<f64, u32> = Builder::new(&it);
